    ...
    public partial class CSWindow : EditorWindow {
        ...
        void OnEnable() {
            ...
            if (palette == null) {
                ...
                if (palette == null) {
                    palette = ScriptableObject.CreateInstance<CSPalette>();
                }
        ...
        void OnGUI() {
            ...
            if (EditorGUI.EndChangeCheck()) {
                if (viewMode == ViewMode.Compact) {
                    ...
                    minSize = new Vector2(minSize.x, 700);
                }
            ...
            if (viewMode == ViewMode.ColorWheel) {
                ...
                palette.scheme = (ColorScheme)GUILayout.SelectionGrid((int)palette.scheme, schemeTexts, 4);
                if (EditorGUI.EndChangeCheck()) {
                    if (prevScheme == palette.scheme) {
                        palette.primaryAngle = UnityEngine.Random.Range(0, Mathf.PI * 2f);
                    } else {
            ...
            if (viewMode == ViewMode.Palette || viewMode == ViewMode.ColorWheel) {
                ...
                EditorGUI.BeginChangeCheck();
                primaryInputMode = (ColorEncoding)EditorGUILayout.EnumPopup(primaryInputMode, GUILayout.Width(100));
                if (EditorGUI.EndChangeCheck()) {
                    UpdatePrimaryInput();
                }

                primaryInput = EditorGUILayout.TextField(GUIContent.none, primaryInput);
            ...
            if (viewMode == ViewMode.ColorWheel || viewMode == ViewMode.Palette || viewMode == ViewMode.Tools) {
                ...
                if (viewMode != ViewMode.Tools) {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    EditorGUI.BeginChangeCheck();
                    selectionInputMode = (ColorEncoding)EditorGUILayout.EnumPopup(selectionInputMode, GUILayout.Width(100));
                    if (EditorGUI.EndChangeCheck()) {
                        UpdateSelectionInput();
                    }

                    selectionInput = EditorGUILayout.TextField(GUIContent.none, selectionInput);
            ...
            if (viewMode == ViewMode.Palette) {
                float width = EditorGUIUtility.currentViewWidth * 0.93f / 3f;
                EditorGUILayout.BeginHorizontal();
